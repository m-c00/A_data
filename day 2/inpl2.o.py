...
interpolated_col_1 = clean_and_interpolate(data_subset1.iloc[:, 0],13550, 13750, 1000, 500, 500)
interpolated_col_1 = clean_and_interpolate(interpolated_col_1,  5300, 5500, 100, 500, 500)

interpolated_col_2 = clean_and_interpolate(data_subset1.iloc[:, 1], 13550, 13750, 1000, 500, 500)
interpolated_col_2 = clean_and_interpolate(interpolated_col_2, 5300, 5500, 100, 500, 500)
...
# Plot original data
ax1.plot(range(len(interpolated_col_1)), interpolated_col_1, label='IN LET', color='blue', linestyle='-')
ax1.plot(range(len(interpolated_col_2)), interpolated_col_2, label='OUT LET', color='green', linestyle='-')
...
# ax2 = ax1.twiny()
# ax2.plot(rolling_avg_inlet.index, rolling_avg_inlet, label='IN LET (Rolling Avg)', color='red', linestyle='--')
# ax2.plot(rolling_avg_outlet.index, rolling_avg_outlet, label='OUT LET (Rolling Avg)', color='black', linestyle='--')
# ax2.set_xlabel('Rolling Average Index')
# ax2.legend(loc='upper right')

# # Plotting the raw data again on a third x-axis
# ax3 = ax1.twiny()
# ax3.plot(data_subset1.index, data_subset1.iloc[:, 0], label='IN LET', color='blue', linestyle='-')
# ax3.plot(data_subset1.index, data_subset1.iloc[:, 1], label='OUT LET', color='green', linestyle='-')
