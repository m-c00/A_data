...
interpolated_col_1_ = fit_and_interpolate(interpolated_col_1, 'gaussian_process', 25920)
interpolated_col_2_ = fit_and_interpolate(interpolated_col_2, 'gaussian_process', 25920)
...
ax1.plot(interpolated_col_2_, label='OUT LET (gaussian_process)', color='blue', linestyle='-')
...
ax2 = ax1.twiny()
x = np.arange(1, len(interpolated_col_1) + 1)  # Assuming both cols have the same length after interpolation
ax2.scatter(x, interpolated_col_1, color='black', label='IN LET Data')
ax2.scatter(x, interpolated_col_2, color='red', label='OUT LET Data')
ax2.set_xlabel('Rolling Average Index')
ax2.legend(loc='upper right')

plt.title('Rolling Average Comparison with Gaussian Process Interpolation')
plt.grid(True)
plt.show()
